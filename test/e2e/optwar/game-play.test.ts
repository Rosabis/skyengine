...
describe("optwar", () => {
  ...
  it("突然加速2", async () => {
    ...
    }
    {
      ...
      // 完整窗口应贴近支付前节奏；5% 仅容纳墙钟和窗口边界抖动。
      expect(resumedRateRatio).toBeGreaterThanOrEqual(0.95)
      expect(resumedRateRatio).toBeLessThanOrEqual(1.05)
      expect(resumedLastFrame.diffPixelCount(resumedFirstFrame)).toBeGreaterThan(0)
      // 两秒末段存在一帧约 5% 的量化误差，10% 边界同时排除冻结和持续漂移。
      expect(resumedTailRateRatio).toBeGreaterThanOrEqual(0.9)
      expect(resumedTailRateRatio).toBeLessThanOrEqual(1.1)
      expect(resumedLastFrame.diffPixelCount(resumedTailFirstFrame)).toBeGreaterThan(0)
      expect(resumedLastFrame.pixel(22, 314)).toEqual([200, 252, 248])
    }
  });
